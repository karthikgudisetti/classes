...
let emp=employees.filter(item=>{
    return item.department==58;
})
console.log(emp);
let e=junior_employees.filter(item=>{
    return item.department==58;
})
console.log(e);

// let l=emp.filter(item=>{
//     return item.name;
// }).map(i=>{
    
//         return i.  junior_assigned
// })
// console.log(l)
...
junior_employees.forEach((item)=>{
    if(emp.junior_assigned.includes(item.employee_id)){
        console.log(`the employee ${emp.name} has ${item.name} as a assisstent`);
    }
});
});


let junior=junior_employees.filter((item)=>{
    return item.department== 58
});

let max=junior.reduce((max,curr)=>{
    return max>curr.products_in_cart[0].cost?max:curr.products_in_cart[0].cost;
}
)
console.log(max);
...
//     if(item.department==58){
//         ssum+=item.salary;
//     }
// })
// junior_employees.forEach(h=>{
//     if(h.department==58){
//         jsum+=h.salary;
//     }
// })
// console.log("it senior sum: "+ssum+" it junior sum: "+jsum);

// let dep=employees.map(item=>{
...
let jsum=0;
    ...
    if(item.department==it.Dcode){
        ssum+=item.salary;
    }
})
junior_employees.forEach(h=>{
    if(h.department==it.Dcode){
        jsum+=h.salary;
    }
})
console.log(`${it.Dname} department  senior salary:  ${ ssum} junior slaary:  ${jsum}`)
})

let sin=employees.map(item=>{
    return item.interests;
})
let jin=junior_employees(item=>{
    return item.interests;
})
console.log(sin);
...
//     console.log(classroom[i][0]);
// }
// console.log("\nfirst row: ");
// for(let i=0;i<classroom.length;i++){
//     console.log(classroom[0][i]);
// }
// console.log("\ndiagonal: ");
// for(let i=0;i<classroom.length;i++){
//     console.log(classroom[i][i]);
// }

// console.log("\n croos diagonal: ");
// for(let i=0;i<classroom.length;i++){
//     console.log(classroom[i][classroom.length-i-1]);
// }
const nested_employees = [
  ...
  [
...

    {
      ...
      junior_assigned: ["JEMP216", "JEMP217", "JEMP218"],
...
// let name=nested_employees.map((emp)=>{
//    emp.map((e)=>{
//     if(e.department==58){
//         return e.name;
//     }
...
// let names = nested_employees
//     .flat()
//     .filter(emp => emp.department === 58)
//     .map(emp => emp.name);

// console.log(names);

nested_employees.forEach((row,col)=>{
    row.forEach((i,j)=>{
        if(i.department==58){
            console.log(i.name);
        }
...
nested_employees.forEach((nest, i) => {
    nest.forEach((emp, j) => {

        const totalAmount = emp.products_in_cart.reduce((prev, curr) => {
            return prev + curr.cost;
        }, 0);

        const percentage = (emp.name.length + emp.name.length) / 100;

        const donation = totalAmount * percentage;

        console.log(
            `Employee: ${emp.name}, TotalAmount: ${totalAmount}, Donation Amount: ${donation.toFixed(2)}`);
    });
});

nested_employees.forEach((innerArray)=>{
    innerArray.forEach((employee)=>{
         employee.junior_assigned.forEach((junior_id)=>{
            let junior = junior_employees.find(jnr=>jnr.employee_id==junior_id)
            if(junior){
                let concatinatedName=junior.name+employee.name;
                console.log(`Concatinated name: ${concatinatedName},percent: ${concatinatedName.length}`)
            }
            else{
                console.log("junior not found");
            }
         })
    })
})

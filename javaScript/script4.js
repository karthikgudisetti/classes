...
let InterestKeys = [
    ...new Set(
        employees.flatMap(emp =>
            emp.interests.flatMap(obj => Object.keys(obj))
        )
    )
];

console.log(InterestKeys);


// let employeeInterestPercentage = employees.map(emp => {
//     let empKeys = emp.interests.map(obj => Object.keys(obj)[0]);

//     return {
//         name: emp.name,
//         percentages: empKeys.map(key => {
//             let index = InterestKeys.indexOf(key);
//             return +(((index + 1) / InterestKeys.length) * 100).toFixed(2);
//         })
//     };
// });

// console.log(employeeInterestPercentage);


// let PerInterest = 100 / InterestKeys.length;

// let result = employees.map(emp => {
//     let total = emp.interests.reduce((sum, obj) => {
//         let key = Object.keys(obj)[0];
//         let index = InterestKeys.indexOf(key);
//         return sum + ((index + 1) / InterestKeys.length) * 100;
//     }, 0);
...
function calculatePercentageForEmployeeAndJuinor(employee){
    let totalPercentageForParticularEmployee=0;
    employee.interests.forEach((interest)=>{
        let key=Object.keys(interest)[0]
        let keyPrecent = InterestKeys.indexOf(key)
        if(keyPercent){
            totalPercentageForParticularEmployee+=KeyPercent;

        }
    })

    console.log("emploeyte",employee.name,totalPercentageForParticularEmployee);
    employee.junior_assigned.forEach(junior)
     employee.interests.forEach((interest)=>{
        let key=Object.keys(interest)[0]
        let keyPrecent = InterestKeys.indexOf(key)
        if(keyPercent){
    ...
    console.log("junior",employee.name, totalPercentageForParticularEmployee)
}
